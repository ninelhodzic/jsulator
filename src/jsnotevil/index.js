...
function FunctionFactory(parentContext){
  ...
  return function Function() {
    ...
    if (typeof src === 'string'){
      ...
      src = parseCode('function a(){' + src + '}').body[0].body
    }
    var tree = prepareAst(src)
...
function prepareAst(src){
  var tree = (typeof src === 'string') ? parseCode(src, {loc: true}) : src
  return hoist(tree)
}
      ...
      switch (node.type) {
        ...
        case 'TryStatement':
          try {
            ...
            enterBlock()
            // changed by Ninel - added .handler instead of .handlers[0]
            var catchClause = node.handler;//.handlers[0]
            if (catchClause) {
        ...
        case 'CallExpression':
          ...
          var applied = target.apply(object, args);
          return checkValue(applied);
